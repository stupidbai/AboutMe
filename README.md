...
> 当前版本：**v4.6.1** ｜ 面向企业客户、方案商、AI 产品团队与产业生态伙伴的个人合作、知识与社区门户。
...
| 案例资产 | 9 个案例，覆盖企业 AI 交付、产教与社群、渠道与生态；WaytoAGI 徐州负责人使用本地高清活动图 |
| 内容资产 | 22 条可运营知识（12 条原创方法卡＋10 条 WayToAGI 公开技术导读）；17 条本地历史知识归档，其中 14 篇本人原创全文与 3 个第三方资料本地引用页 |
...
| 版本 | 核心更新 |
| --- | --- |
| v4.6.1 | 知识库问答在每次提问前重新校验模型状态，并增加“刷新模型状态”入口；京东云 GLM 连接会关闭深度推理，保证页面取得最终回答而不展示推理内容 |
| v4.6.0 | 首页强化案例/合作入口与直接合作路径；5 张大图改为受控 WebP；管理端新增每天基于真实匿名数据生成的可解释优化建议；保留加密的 OpenAI Chat Completions 兼容 AI 配置能力 |
| v4.5.2 | Docker Compose 镜像标签与应用版本、镜像元数据统一为 `4.5.2`，保证部署版本可追溯 |
| v4.5.1 | 首页「AI 社群矩阵覆盖用户」更新为 `1000W+`；数据库一次性迁移同步既有线上配置，避免历史快照覆盖新指标 |
| v4.5.0 | 四段职业经历精确到月；新增 10 条带来源记录的 WayToAGI 技术导读；知识 CMS 支持配置来源名称与链接，数据库升级到 v8 并自动迁移线上旧数据 |
| v4.4.0 | AAIA AIGC 委员会理事及聘书调整为可配置的“其他专业与组织背书”；管理端访问监控新增明确的每日、每月 PV/UV/会话统计，并可查看近 365 天月度趋势 |
| v4.3.0 | AAIA AIGC 委员会理事及聘书从案例迁入可配置履历时间线；全站视觉调整为蓝色主调、紫色渐变点缀；案例恢复为 9 个 |
...
- **首页与合作导航**：以目录化路径引导访客了解经历、合作场景、案例、知识和联系入口；首屏保留“查看合作案例 / 发起合作沟通”，并给直接访问者提供“看案例—说场景—对合作方式”的短路径。
- **履历与合作页**：呈现职业时间线、专业与组织背书、企业 AI / 可信数字化 / FDE / 生态合作方向及合作推进方式。
- **案例页**：按产品与工程交付、社群与人才、渠道与生态分类展示；案例可配置 NAS 或项目资料地址，公众仅可访问、不可修改。
- **知识库**：支持分类、检索、详情页、当前方法卡、公开来源技术导读与本地历史归档；导读正文留在本站，原始链接仅作为可核验的来源记录。
...
- 已支持将管理员明确配置的京东云 `agentrs.jd.com` Chat Completions 接口与 `GLM-5.2` 用于 RAG 问答；因部分 DNS 会将该官方地址映射到基准测试地址段，安全放行被限制为该 HTTPS 完整路径，不能扩展到任意内网或自定义地址。
- API Key、SMTP 密码及 Turnstile 服务端密钥使用 AES-256-GCM 加密后保存；默认阻断模型接口访问回环和内网地址。

### 访问监控与数据分析

- 管理端提供近 7 / 30 / 90 / 365 天 PV、UV、会话、互动率、回访、页面表现、来源、设备、行动转化与性能 P95；按自然日和自然月分别汇总访问量，并每天从实际匿名数据生成可解释的优化建议。
...
- [config/cases.json](config/cases.json)：9 个案例的静态种子和 GitHub Pages 回退数据。
- [config/site-config.json](config/site-config.json)：身份、首页、联系、职业时间线、专业与组织背书与合作内容种子。
- [config/knowledge.json](config/knowledge.json)：当前知识方法卡种子。
- [docs/waytoagi-knowledge-sources.json](docs/waytoagi-knowledge-sources.json)：10 条 WayToAGI 技术导读的抓取日期、原始链接与内容边界清单。
...
- SQLite、自动生成的加密密钥和轮换备份位于 `data/`；升级时必须保留该目录或 Docker 数据卷。AI API Key 只以 AES-256-GCM 密文保存，绝不应提交到 Git 或写入静态配置。
