...
import type { CooperationDirection, CredentialItem, Metric, PortalRoute, SiteConfig } from '../../../data/siteConfig'
...
const addTimeline = () => config.value?.timeline.push({ period: '时间', organization: '组织名称', role: '角色', description: '请填写主要职责与成果。', current: false, type: 'career', image: '', imageAlt: '' })
const addCredential = () => config.value?.credentials.push({ period: '专业背书', organization: '组织名称', title: '背书标题', description: '请填写专业认可、组织任职或代表性成果。', image: '', imageAlt: '' } as CredentialItem)
...
<template>
  <section class="case-admin site-admin">
    ...
    <template v-else-if="config">
      ...
      <details class="site-admin-section"><summary>职业时间线（{{ config.timeline.length }}）</summary><div class="site-admin-repeat"><article v-for="(item, index) in config.timeline" :key="index" class="case-admin-card"><header><strong>{{ item.period }} · {{ item.organization }}</strong><div class="case-admin-card__actions"><button @click="move(config.timeline, index, -1)">↑</button><button @click="move(config.timeline, index, 1)">↓</button><button class="danger" @click="remove(config.timeline, index)">删除</button></div></header><div class="case-admin-fields"><label><span>时间</span><input v-model="item.period"></label><label><span>组织</span><input v-model="item.organization"></label><label><span>记录类型</span><select v-model="item.type"><option value="career">职业经历</option><option value="appointment">行业任职</option></select></label><label class="wide"><span>角色</span><input v-model="item.role"></label><label class="wide"><span>说明</span><textarea v-model="item.description" rows="2"></textarea></label><label class="wide"><span>凭证图片路径（可选）</span><input v-model="item.image" placeholder="/assets/..."></label><label class="wide"><span>图片说明（可选）</span><input v-model="item.imageAlt"></label><label class="case-admin-check"><input v-model="item.current" type="checkbox"><span>标记为当前经历</span></label></div></article><button class="site-admin-add" @click="addTimeline">＋ 新增经历 / 任职</button></div></details>

      <details class="site-admin-section"><summary>其他专业与组织背书（{{ config.credentials.length }}）</summary><div class="site-admin-repeat"><article v-for="(item, index) in config.credentials" :key="index" class="case-admin-card"><header><strong>{{ item.organization }} · {{ item.title }}</strong><div class="case-admin-card__actions"><button @click="move(config.credentials, index, -1)">↑</button><button @click="move(config.credentials, index, 1)">↓</button><button class="danger" @click="remove(config.credentials, index)">删除</button></div></header><div class="case-admin-fields"><label><span>时间 / 分类</span><input v-model="item.period"></label><label><span>组织</span><input v-model="item.organization"></label><label class="wide"><span>背书标题</span><input v-model="item.title"></label><label class="wide"><span>说明</span><textarea v-model="item.description" rows="2"></textarea></label><label class="wide"><span>凭证图片路径（可选）</span><input v-model="item.image" placeholder="/assets/..."></label><label class="wide"><span>图片说明（可选）</span><input v-model="item.imageAlt"></label></div></article><button class="site-admin-add" @click="addCredential">＋ 新增专业与组织背书</button></div></details>
