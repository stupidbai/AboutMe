...
interface Post { id: string; categoryId: string; categoryName: string; title: string; excerpt: string; bodyHtml?: string; status: string; viewCount: number; replyCount: number; likeCount: number; viewerLiked: boolean; createdAt: string; lastActivityAt: string; author: Author | null }
...
const loadPosts = async () => {
  const params = new URLSearchParams(); if (filter.category) params.set('category', filter.category); if (filter.q.trim()) params.set('q', filter.q.trim())
  try { const response = await fetch(`/api/forum/posts?${params}`, { credentials: 'same-origin' }); if (!response.ok || !response.headers.get('content-type')?.includes('application/json')) throw new Error(); posts.value = (await response.json()).posts; available.value = true } catch { available.value = false }
}
...
const createPost = async () => { busy.value = 'post'; error.value = ''; try { const response = await fetch('/api/forum/posts', { method: 'POST', credentials: 'same-origin', headers: { 'content-type': 'application/json' }, body: JSON.stringify(draft) }); if (!response.ok) { error.value = await readError(response); return }; const post = await response.json(); draft.title = ''; draft.body = ''; showComposer.value = false; await loadCategories(); await loadPosts(); await openPost(post.id) } finally { busy.value = '' } }
const sendReply = async () => { if (!selected.value) return; busy.value = 'reply'; error.value = ''; try { const response = await fetch(`/api/forum/posts/${selected.value.id}/replies`, { method: 'POST', credentials: 'same-origin', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ body: reply.value }) }); if (!response.ok) { error.value = await readError(response); return }; reply.value = ''; await openPost(selected.value.id) } finally { busy.value = '' } }
const like = async (type: 'posts' | 'replies', item: Post | Reply) => { if (!state.user) return; const response = await fetch(`/api/forum/${type}/${item.id}/like`, { method: 'POST', credentials: 'same-origin' }); if (response.ok) Object.assign(item, await response.json()) }
const remove = async (type: 'posts' | 'replies', item: Post | Reply) => { if (!confirm('确认删除该内容？')) return; const response = await fetch(`/api/forum/${type}/${item.id}`, { method: 'DELETE', credentials: 'same-origin' }); if (!response.ok) { error.value = await readError(response); return }; if (type === 'posts') { closePost(); await loadPosts() } else if (selected.value) await openPost(selected.value.id) }
</script>
...
<template>
  <section class="forum-board">
    ...
    <template v-else>
      ...
      <div class="forum-categories"><button :class="{ active: !filter.category }" @click="filter.category = ''; loadPosts()"><strong>全部</strong><span>{{ posts.length }} 条当前结果</span></button><button v-for="category in categories" :key="category.id" :class="{ active: filter.category === category.id }" @click="filter.category = category.id; loadPosts()"><strong>{{ category.name }}</strong><span>{{ category.postCount }} 篇 · {{ category.description }}</span></button></div>
      <form class="forum-search" @submit.prevent="loadPosts"><input v-model="filter.q" maxlength="100" placeholder="搜索标题与正文"><button>搜索</button></form>
      <h2 class="forum-result-title">{{ resultLabel }}</h2><div class="forum-post-list"><button v-for="post in posts" :key="post.id" class="forum-post" @click="openPost(post.id)"><div><span>{{ post.categoryName }}</span><h3>{{ post.title }}</h3><p>{{ post.excerpt }}</p><small>{{ post.author?.displayName || '已注销用户' }} · {{ formatDate(post.lastActivityAt) }}</small></div><dl><div><dt>{{ post.replyCount }}</dt><dd>回复</dd></div><div><dt>{{ post.viewCount }}</dt><dd>浏览</dd></div></dl></button><div v-if="!posts.length" class="community-empty">暂时没有匹配的讨论。</div></div>
    </template>
