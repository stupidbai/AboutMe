      ...
      try {
        this.database.prepare(`
          UPDATE ai_settings SET enabled = ?, provider = ?, api_url = ?, model = ?,
            ...
            max_tokens = ?, system_prompt = ?, revision = ?, updated_at = ?, updated_by = ? WHERE id = 1
        `).run(incoming.enabled ? 1 : 0, incoming.provider, incoming.apiUrl, incoming.model,
          ...
          incoming.maxTokens, incoming.systemPrompt, revision, now, String(actor).slice(0, 100))
