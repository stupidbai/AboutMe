...
本系统使用 Node.js 内置 SQLite。运行时不需要额外数据库服务，Windows、Linux 与 Docker 使用相同的数据结构和管理 API。v3.9.0 使用 MiniSearch 7.2.0 提供本地分段检索，并支持知识库配置、问答反馈/统计与 OpenAI 兼容 AI 问答。
...
1. 解压 bai-yunfei-portal-v3.9.0.zip。
2. 在 PowerShell 中运行：
...
~~~bash
tar -xzf bai-yunfei-portal-v3.9.0.tar.gz
cd bai-yunfei-portal-v3.9.0
...
数据库和自动生成的加密密钥保存在命名卷 bai-yunfei-portal-data 中，重新构建镜像或删除容器不会丢失案例、知识和站点配置。除非明确要删除所有业务数据，不要执行 docker compose down -v。Compose 要求显式设置 `CASE_ADMIN_PASSWORD`，不再使用默认弱密码。
...
- SQLite v4 保存问答编号、脱敏访问来源、问题、响应模式、命中数量、耗时与用户反馈，用于管理页统计；不保存访问者姓名或联系方式。
